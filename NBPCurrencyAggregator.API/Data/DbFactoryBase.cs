...
namespace NBPCurrencyAggregator.API.Data
{
    public abstract class DbFactoryBase
    {
        ...
        public virtual async Task<Currency> DbQueryCurrencyFromTo(string sql, object parameters = null)
        {
            var lookup = new Dictionary<int, Currency>();
            using IDbConnection dbCon = DbConnection;
            var result = parameters == null
                ? await dbCon.QueryAsync<Currency, Rate, Currency>(sql, (currency, rate) =>
                    {
                        Currency curr;
                        if (!lookup.TryGetValue(currency.Id, out curr))
                        {
                            lookup.Add(currency.Id, curr = currency);
                        }

                        if (curr.Rates == null)
                            curr.Rates = new List<Rate>();
                        curr.Rates.Add(rate);
                        return curr;
                    }
                )
